...
int main()
{
	...
	// take some input for user to tell us -> char
	...
	while (go_again == 'y') {
        ...
        switch (option) {
            ...
            case MAX_POWER:
                printf("Enter the amount of voltage (V): ");
                scanf("%f", &voltage);
                printf("Enter the amount of resistance (R): ");
                scanf("%f", &resistance);

                double max_power = (voltage * voltage) / (4 * resistance);
                printf("Max power: %.2lf\n", max_power);
                break;
            // input v
            default:
        ...
        printf("Would you like to go again? If so enter y");
        scanf(" %c", &go_again);
	}
